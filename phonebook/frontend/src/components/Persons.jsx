...
const Persons = ({persons, setPersons, filter}) => {
  ...
  return (
    newPersons.map(person => <Person key={person.id} person={person} handleDelete={onClick} />)
  )
}

const Person = ({person, handleDelete}) => (
    <p>{person.name} {person.number}<button type="submit" onClick={() => handleDelete(person)}>delete</button></p>
)
